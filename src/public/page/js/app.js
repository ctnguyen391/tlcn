            ...
            _this.cart.forEach(item => {
                items +=
                    ` <div class="ps-product--mini-cart">
                            <div class="ps-product__thumbnail"><a href="#"><img src="${item.thumbnail}"
                                alt=""></a></div>
                            <div class="ps-product__content"><span class="ps-product__remove remove-mini-cart-item" data-product-id="${item.id}"><i class="icon-cross"></i></span><a
                                class="ps-product__title" href="/san-pham">${item.title}</a>
                            <p> Qty: ${item.qty}</p><span class="ps-product__price">${formatPrice(item.price)}</span>
                            </div>
                        </div>`;
            })
            eleMiniCartItems.innerHTML = items;
            eleRemoveMiniCartItem = document.querySelectorAll('.remove-mini-cart-item') ?? [];
            ...
            _this.cart.forEach(item => {
                ...
                <td>
                  <div class="ps-product--sidebar">
                    ...
                    <div class="ps-product__content"><a class="ps-product__title" href="san-pham/${item.id}">${item.title}</a>
                    </div>
                  </div>
                </td>
                ...
                <td>
                  <div class="form-group--number">
                    <input onkeyup="cart.updateCartMain(${item.id})" class="form-control" type="text" value="${item.qty}">
                  </div>
                </td>
                <td>
                  <p><strong>${formatPrice(item.price * item.qty)}</strong></p>
                </td>
                <td class="remove-mini-cart-item" data-product-id="${item.id}"><a class="ps-icon"><i class="icon-cross"></i></a></td>
              </tr>`;
            });
            ...
            cartCheckout.addEventListener('click', function () {
                fetch('/checkout', {
                    ...
                    .then(data => {
                        ...
                        eleCartContent.innerHTML = "<div class='alert alert-primary'>Đặt hàng thành công, chúng tôi sẽ liên hệ lại với bạn trong thời gian sớm nhất</div>"
                        _this.removeAll();
                        _this.renderCartHeader();
                    })
                    .catch((error) => {
